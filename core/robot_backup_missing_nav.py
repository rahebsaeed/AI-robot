...
class Robot:
    ...
    Manual movement:
      AI -> UDP 5020 -> Docker bridge -> /cmd_vel

    Map navigation:
      AI -> Docker ROS -> /move_base_simple/goal

    This allows:
      - normal voice movement
      - map-based search using AMCL + move_base
      - RViz path/pose tracking
    """
...
"""
    ...
    def control_arm(self, command="home"):
        ...
        # Important: searching movement is controlled by search_tasks.py,
        # not by arm command.
...
rospy.init_node('ai_send_move_base_goal', anonymous=True)
...
"""
    ...
    def wait_until_near(self, x: float, y: float, radius: float = 0.45, timeout: float = 35.0) -> bool:
        start = time.time()
        ...
        while time.time() - start < timeout:
            ...
            if d is not None:
                print(f"[ROBOT NAV]: distance to goal = {d:.2f} m", flush=True)
