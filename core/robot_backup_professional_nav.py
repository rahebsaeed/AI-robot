...
""",
    ...
    def goto_map(self, x: float, y: float, yaw: float = 0.0) -> bool:
        """
        Send a real move_base action goal in map frame.

        This is stronger than publishing /move_base_simple/goal because it talks
        directly to the move_base action server.
        """
        ...
        print(f"[ROBOT NAV]: sending MOVE_BASE ACTION goal x={x:.2f}, y={y:.2f}, yaw={yaw:.2f}", flush=True)
...
rospy.init_node('ai_move_base_action_goal', anonymous=True)

client = actionlib.SimpleActionClient('/move_base', MoveBaseAction)

print('WAITING_FOR_MOVE_BASE')
ok = client.wait_for_server(rospy.Duration(5.0))

if not ok:
    print('MOVE_BASE_SERVER_NOT_AVAILABLE')
    raise SystemExit(0)
...
PY
"""
        out = self._docker_ros(cmd, timeout=10)

        if "ACTION_GOAL_SENT" in out:
            print("[ROBOT NAV]: action goal sent successfully", flush=True)
            return True

        print(f"[ROBOT NAV ERROR]: goal not accepted. Output: {out}", flush=True)
        return False
