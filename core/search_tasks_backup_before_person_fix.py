...
class MapSearchTask:
    ...
    def target_visible(self, target_text: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            vision = {}
            print(f"[SEARCH] perception error: {e}", flush=True)
        ...
        if yolo_name:
            for obj in objects:
                ...
                if name == yolo_name and conf >= 0.25:
                    return True, f"found {yolo_name} confidence={conf:.2f}"
    ...
    def scan_at_current_place(self, target_text: str, seconds=6.0) -> Tuple[bool, str]:
        """
        Professional mode:
        Do not fake-search using manual /cmd_vel left/right turns.

        Searching should be done by map navigation and camera checks.
        If we need to rotate in place later, it must be done through move_base
        or a proper ROS navigation behavior, not short UDP pulses.
        """
        print(f"[SEARCH] local scan skipped in professional navigation mode for {target_text}", flush=True)
        ...
        return False, "not found in current camera view"
