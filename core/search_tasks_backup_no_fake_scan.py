...
class MapSearchTask:
    ...
    def scan_at_current_place(self, target_text: str, seconds=6.0) -> Tuple[bool, str]:
        print(f"[SEARCH] camera/body scan for {target_text}", flush=True)

        start = time.time()

        # small scan right
        self.robot.move("right", speed=0.28, duration=seconds / 2.0)
        ...
        # small scan left
        self.robot.move("left", speed=0.28, duration=seconds / 2.0)
        ...
        self.robot.stop()
        return False, "not found in local scan"
